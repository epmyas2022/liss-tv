    ...
    <>
      ...
      <div className="hidden md:block fixed top-6 left-1/2 -translate-x-1/2 z-50 w-max">
        <nav
          ...
          style={glassStyle}
        >
          <Link href="/" className="font-poppins px-6 text-white text-2xl font-bold shrink-0">
          ...
          {NAV_ITEMS.map(({ label, href, icon: Icon }) => (
            <Link
              ...
              style={{
                ...
                background: active === label ? "rgba(234,28,37,0.85)" : "transparent",
                boxShadow: active === label ? "0 2px 12px rgba(234,28,37,0.4)" : "none",
              }}
              onMouseEnter={(e) => {
                if (active !== label) {
          ...
          <div className="px-3 relative">
            <input
              ...
              style={{
                border: pending ? "2px solid #EA1C25" : "2px solid rgba(209,213,219,0.6)",
            ...
            {pending && (
              <span
                ...
                style={{ borderTopColor: "#EA1C25", animation: "spin 0.6s linear infinite" }}
              />
            )}
          </div>
        </nav>
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                <input
                  ...
                  style={{
                    border: pending ? "2px solid #EA1C25" : "2px solid rgba(209,213,219,0.5)",
                  }}
                  onChange={(e) => handleSearch(e.target.value)}
                />
                {pending && (
              ...
              <button onClick={() => setSearchOpen(false)} className="text-white/70 hover:text-white">
                <X size={20} />
              </button>
            </div>
          ) : (
            <button onClick={openSearch} className="text-white/70 hover:text-white p-1">
              <Search size={20} />
            </button>
          )}
        </div>
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 z-50" style={glassStyle}>
